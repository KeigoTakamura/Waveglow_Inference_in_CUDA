...
{   
    ...
    {
        hipLaunchKernelGGL(copy_kernel, dim3((audio_0.size()+n_threads-1)/n_threads), dim3(n_threads ), 0, 0, audio_0.size(), input_t.ptr, audio_0.ptr);
        ...
        for(int j=0; j<n_layers; j++)
        {
                ...
                hipLaunchKernelGGL(fused_add_tanh_sigm_mul, dim3((gated_activation_output.size()+n_threads-1)/n_threads), dim3(n_threads ), 0, 0, gated_activation_output.size(), in_conv_out.ptr, f3.ptr, gated_activation_output.ptr);
                ...
                if(j<7)
                {
                    ...
                    hipLaunchKernelGGL(skip_res_add, dim3((f1.size()+n_threads-1)/n_threads), dim3(n_threads ), 0, 0, f1.size(), f3.ptr, f1.ptr, skip_out_sum.ptr, 256*input_len);
                }
                else
                {
                    hipdnnSetTensor4dDescriptor(input_desc, hipdnnTensorFormat_t::HIPDNN_TENSOR_NCHW, hipdnnDataType_t::HIPDNN_DATA_FLOAT, 1, n_channels, 1, input_len);
                    ...
                    hipLaunchKernelGGL(skip_add, dim3((f1.size()+n_threads-1)/n_threads), dim3(n_threads ), 0, 0, f1.size(), f1.ptr, skip_out_sum.ptr);
                }
        ...
        hipLaunchKernelGGL(affine_transform, dim3((audio.size()/2+n_threads-1)/n_threads), dim3(n_threads ), 0, 0, audio.size()/2, input_t.ptr, audio.ptr, audio.size()/2);
        ...
        hipLaunchKernelGGL(copy_kernel, dim3((input_t.size()+n_threads-1)/n_threads), dim3(n_threads), 0, 0, input_t.size(), audio.ptr, input_t.ptr);
        ...
        if((k%4==0) && (k>0))
            {
                ...
                hipLaunchKernelGGL(concat_z, dim3((input_t.size()+n_threads-1)/n_threads), dim3(n_threads), 0, 0, input_t.size(), audio.ptr, input_t.ptr, z.ptr, 2*input_len);
    ...
    hipLaunchKernelGGL(transpose_kernel, dim3((d_output.size()+n_threads-1)/n_threads), dim3(n_threads), 0, 0, d_output.size(), input_t.ptr, d_output.ptr, input_t.shape[1], input_t.shape[0]);
