...
{   
    ...
    // hipLaunchKernelGGL(fractional_stride_nchw, dim3((num_values+n_threads-1)/n_threads), dim3(n_threads), 0, 0, num_values, stride, input_mel.ptr, f1.ptr, input_len, input_rows);
    ...
    num_values = f1.size();
    hipLaunchKernelGGL(reshape, dim3((num_values+n_threads-1)/n_threads), dim3(n_threads), 0, 0, num_values, f2.ptr, f1.ptr, output_rows, upsampled_dim);

    f2.reshape(640, upsampled_dim/8);
    hipLaunchKernelGGL(transformation, dim3((num_values+n_threads-1)/n_threads), dim3(n_threads), 0, 0, num_values, f1.ptr, d_output.ptr, upsampled_dim, upsampled_dim/8);
}
